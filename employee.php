...
echo"
                            ...
                            <tbody>";
                            include("connect.php");
                            $sql = "SELECT email,role,password,address,username,phone FROM customer where role='3'";
                            ...
                            while ($row = $result->fetch_assoc()) {
                              ...
                              echo "<TD><a href=employee.php?op=1&email=$email><button type='button' class='btn btn-primary'>修改 <i class='bi bi-alarm'></i></button></a>";
                              echo "<TD><a href=\"javascript:if(confirm('確實要刪除[$email]嗎?'))location='employee.php?email=$email&op=5'\"><button type='button' class='btn btn-danger'>刪除 <i class='bi bi-trash'></i></button>";
                            }
tf();
footer();
...
function display_form($op,$email)
  {
    ...
    else
    {
      ...
      echo "<div class='modal-body'><form action=employee.php method=post>";
