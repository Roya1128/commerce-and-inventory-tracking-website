...
echo"
                            ...
                            <tbody>";
                            include("connect.php");
                            $sql = "SELECT prodname,unitprice,kind FROM inv ";
                            ...
                            while ($row = $result->fetch_assoc()) {
                              //printf("%s (%s)\n", $row["Name"], $row["CountryCode"]);
                                ...
                                $kind=$row['kind'];
                              echo "<tr><TD>$prodname<td>$kind<TD>$unitprice";    
                              echo "<TD><a href=product.php?op=1&prodname=$prodname><button type='button' class='btn btn-primary'>修改 <i class='bi bi-alarm'></i></button></a>";
                              echo "<TD><a href=\"javascript:if(confirm('確實要刪除[$prodname]嗎?'))location='product.php?prodname=$prodname&op=5'\"><button type='button' class='btn btn-danger'>刪除 <i class='bi bi-trash'></i></button>";
                            }
tf();
footer();
...
function display_form($op,$prodname)
  {
    ...
    {
        include("connect.php");
        $sql = "SELECT prodname,unitprice,kind FROM inv where prodname='$prodname'";
      ...
      echo "<div class='modal-body'><form action=product.php method=post>";
      ...
      echo "<div class='mb-3'>
            ...
            <div class='mb-3'>
              ...
              <input type='text' class='form-control' name=kind id='kind' placeholder='請輸入種類' value=$kind>
            </div>
            ";
       ...
       switch ($op)
       {
         ...
         case 4: //新增資料
            ...
            $kind=$_REQUEST['kind'];
               ...
               $sql="insert into inv (unitprice,prodname,kind) 
               values ($unitprice,'$prodname','$kind')";
               include("connect.php");
               include("dbutil.php");
               include("basic.php");
               execute_sql($sql);
